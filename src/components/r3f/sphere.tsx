...
const SphereContainer = () => {
  return (
    ...
    >
      <fogExp2 attach="fog" args={[0x060209, 0.08]} />
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < total; i++) {
      ...
      for (let j = 0; j < total; j++) {
        ...
        if (
          currentPoint.current.distanceTo(target) < 1 &&
          state.pointer.x !== 0 &&
          state.pointer.y !== 0
        ) {
          lerped = new THREE.Vector3(baseX, baseY, baseZ).lerp(
            currentPoint.current,
            THREE.MathUtils.clamp(
              -currentPoint.current.distanceTo(target),
              -0.1,
              1,
            ),
          );
